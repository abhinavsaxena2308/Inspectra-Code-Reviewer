...
export const DashboardPage = () => {
  ...
  return (
    <div className="p-8 space-y-8 max-w-7xl mx-auto w-full">
      ...
      {/* Recent Intelligence Sequences (Activity) */}
      <section className="space-y-6">
        <div className="flex items-center justify-between">
          <h2 className="text-xl font-bold tracking-tight text-on-surface">Recent Intelligence Sequences</h2>
          <button 
        ...
        <div className="bg-surface-container-low rounded-xl overflow-hidden shadow-sm">
          <div className="divide-y divide-surface-container-highest">
            {activities.slice(0, 5).map((activity) => (
              <div key={activity.id} className="p-4 flex items-center justify-between hover:bg-surface-container transition-colors group cursor-pointer" onClick={() => navigate(`/analysis/${activity.id}`)}>
                <div className="flex items-center gap-4">
                  <div className={cn(
                    "w-8 h-8 rounded-lg flex items-center justify-center shrink-0",
                    activity.type === 'analysis-completed' ? 'bg-secondary/10 text-secondary' : 'bg-primary/10 text-primary'
                  )}>
                    {activity.type === 'analysis-completed' ? <CheckCircle2 className="w-4 h-4" /> : <Loader2 className="w-4 h-4 animate-spin" />}
                  </div>
                  <div>
                    ...
                    <p className="text-[10px] text-outline uppercase tracking-tighter">
                      {activity.type === 'analysis-completed' ? 'Deep Scan Complete' : 'Analysis In Progress'} • {activity.timestamp}
                    </p>
                  </div>
                </div>
                <ArrowUpRight className="w-4 h-4 text-outline group-hover:text-on-surface transition-colors" />
              </div>
            ))}
          </div>
          {activities.length === 0 && (
            <div className="p-12 text-center">
              <p className="text-sm text-outline">No recent activity detected.</p>
            </div>
          )}
        </div>
      </section>
